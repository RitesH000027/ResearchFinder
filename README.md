...
2. **Citation Database (Secondary Machine)**
...
├── SECONDARY_SETUP.md                # Secondary machine setup guide
...
### Secondary Machine Setup (Citation Server)

**Note**: Secondary machine files are documented in `SECONDARY_SETUP.md` but not included in the main codebase to keep it clean.
...
2. **Download and set up citation server files:**
   - Get `citation_server.py` from the README examples or releases
   - Get `load_citations.py` for data loading
   - Get `citation_database_schema.sql` for database setup

3. **Load citation data and start server:**
```bash
python load_citations.py
python citation_server.py
```

The server will start on `http://0.0.0.0:5000` and provide these endpoints:
...
4. **Update the citation server IP in citation_analysis.py:**
```python
# Update this line with your secondary machine's IP
...
```python
# Change this to your secondary machine's IP address
...
Citation database (`scripts/citation_server.py`):
```python
DB_CONFIG = {
    'host': 'localhost',
...
```bash
python diagnose_citations.py
```

3. **Full System Test:**
```bash
python test_fixed_citations.py
```
...
**Solutions**:
- Check if citation server is running: `python scripts/citation_server.py`
